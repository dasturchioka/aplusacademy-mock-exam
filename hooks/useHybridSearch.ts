...
export function useHybridSearch<T>({
	...
	const [isSearching, setIsSearching] = useState(false)          // Server refetch indicator after first load
		...
		const fetchData = async () => {
			try {
				if (!hasFetchedOnce.current) {
					...
					// After the first fetch, keep refetch feedback local to the data surface.
					setIsSearching(true)
				}
	...
	return {
		...
		isSearching,                        // Server refetches after first load
