...
export default function EditTestPage() {
	...
	const saveTestData = async () => {
		...
		try {
			...
			if (result.success) {
				setHasChanges(false)
				notify.success('Test saved', 'Changes have been saved successfully.')
			} else {
				const errorMessage = result.error || 'Failed to save test'
				setError(errorMessage)
				notify.error('Save failed', errorMessage)
			}
		} catch (err: any) {
			const errorMessage = err.message || 'Failed to save test'
			setError(errorMessage)
			notify.error('Save failed', errorMessage)
				...
				<CardHeader className='pb-3'>
					<div className='flex items-center justify-between'>
						<div className='flex items-center gap-2'>
							<StatusBadge status='neutral' label={`Q${questionIndex + 1}`} />
							<Select
		...
		return (
			<div className='space-y-6'>
				<div className='flex items-center gap-2 mb-4'>
					...
					<StatusBadge status='info' label={`${data.parts.length} parts`} />
				</div>
	...
	// Loading state
	if (isLoading) {
		return <PageState type='loading' title='Loading test editor' className='container mx-auto max-w-6xl p-6' />
	}
	...
	if (error || !testData) {
		return (
			<div className='container mx-auto p-6 max-w-6xl'>
				<AppAlert tone='error' title='Unable to load test'>
	...
	return (
		<div className='container mx-auto p-6 max-w-6xl'>
			...
			<div className='flex items-center justify-between mb-8'>
				...
				<div className='flex items-center gap-2'>
					{hasChanges && (
						<StatusBadge status='pending' label='Unsaved changes' />
					)}
					<LoadingButton
						type='button'
			...
			<Tabs value={activeTab}>
				<TabsList className='grid w-full grid-cols-3'>
					<TabsTrigger value='listening' disabled={!testData.listening}>
						...
						Listening
						{testData.listening && <CheckCircle2 className='w-4 h-4 ml-2 text-muted-foreground' />}
					</TabsTrigger>
					<TabsTrigger value='reading' disabled={!testData.reading}>
						<FileText className='w-4 h-4 mr-2' />
						Reading
						{testData.reading && <CheckCircle2 className='w-4 h-4 ml-2 text-muted-foreground' />}
					</TabsTrigger>
					<TabsTrigger value='writing' disabled={!testData.writing}>
						<PenTool className='w-4 h-4 mr-2' />
						Writing
						{testData.writing && <CheckCircle2 className='w-4 h-4 ml-2 text-muted-foreground' />}
					</TabsTrigger>
				</TabsList>
				...
				<TabsContent value='listening' className='mt-6'>
					{testData.listening ? (
						...
						<AppAlert tone='warning' title='Listening section unavailable'>
							Process a listening test first.
						</AppAlert>
					)}
				</TabsContent>
				...
				<TabsContent value='reading' className='mt-6'>
					{testData.reading ? (
						...
						<AppAlert tone='warning' title='Reading section unavailable'>
							Process a reading test first.
						</AppAlert>
					)}
				</TabsContent>
				...
				<TabsContent value='writing' className='mt-6'>
					{testData.writing ? (
						...
						<AppAlert tone='warning' title='Writing section unavailable'>
							Process a writing test first.
						</AppAlert>
					)}
				</TabsContent>
			</Tabs>
