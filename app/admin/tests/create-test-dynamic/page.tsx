        ...
        <div className="flex items-center justify-between p-4 pb-2">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary">
              Q{block.questionStart}-{block.questionEnd}
            </Badge>
            <Button
              variant="ghost"
              size="sm"
...
export default function CreateTestDynamic() {
    ...
    if (section === "listening") {
      setTestData((prev) => {
        ...
        if (!updated.listening.parts[partIndex].questionBlocks) {
          console.warn(`⚠️ Initializing questionBlocks array for Listening Part ${partIndex + 1}`);
      ...
      setTestData((prev) => {
        ...
        if (!updated.reading.passages[partIndex].questionBlocks) {
          console.warn(`⚠️ Initializing questionBlocks array for Reading Passage ${partIndex + 1}`);
  ...
  const saveTestToSupabase = async () => {
    ...
    try {
      ...
      const response =
        isEditMode && testId
          ? await axios.put(`/api/tests/${testId}`, payload)
      ...
      setSuccess(
        isEditMode
          ...
          : `Test saved successfully to Supabase!${sectionsText}`
      );

      // Only reset form if ALL sections are complete (not a partial save)
      const isCompleteSave = hasListeningContent && hasReadingContent && hasWritingContent;

      // Clear localStorage draft after successful complete save (only for create mode)
      if (!isEditMode && isCompleteSave) {
        localStorage.removeItem("ielts-test-draft");
      }

      // Reset form after successful complete save (only for create mode and complete test)
      // For partial saves, keep the form open so user can continue working
      if (!isEditMode && isCompleteSave) {
        setTimeout(() => {
          setShowForm(false);
          setTestData({
      ...
      } else if (!isEditMode && !isCompleteSave) {
        // For partial saves, show a message that user can continue
        setTimeout(() => {
          setSuccess(
            `Test saved successfully!${sectionsText}\nYou can continue adding the remaining sections and save again.`
          );
        }, 2000);
      }
    } catch (error: any) {
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant="outline">
              {testData.listening_audios?.length > 0 ? "Uploaded" : "No audio"}
            </Badge>
          </CardTitle>
        </CardHeader>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            input.onchange = async (e) => {
                              ...
                              // Pass ALL existing audios to be deleted
                              formData.append(
                                "existingAudios",
                                JSON.stringify(testData.listening_audios || [])
                              );
                              ...
                              try {
                                ...
                                // Upload new audio (server will delete all existing ones)
                                const uploadRes = await axios.post(
                                  "/api/upload-listening-audio",
                                  formData,
                                  {
                                    headers: {
                                ...
                                if (
                                  ...
                                  // Replace entire array with single new audio
                                  setTestData((prev) => ({
                                    ...prev,
                                    listening_audios: [uploadRes.data.urls[0]],
                                  }));
                            ...
                            if (confirm(`Are you sure you want to delete Audio ${index + 1}?`)) {
                              try {
                                ...
                                try {
                                  ...
                                  console.log("✅ Audio file deleted from server:", url);
                                } catch (deleteErr) {
                                  console.warn("Failed to delete audio from server:", deleteErr);
                                  // Continue anyway to remove from state
                                }

                                // Remove from state
                                const newAudios = testData.listening_audios.filter((_, i) => i !== index);
                                setTestData((prev) => ({
                                  ...prev,
                                  listening_audios: newAudios,
                                }));
              ...
              input.onchange = async (e) => {
                ...
                try {
                  ...
                  if (res.data.success && res.data.urls?.[0]) {
                    ...
                    setTestData((prev) => ({
                      ...prev,
                      listening_audios: [res.data.urls[0]],
                    }));
                    const message =
                      res.data.deletedCount > 0
                        ? `Audio uploaded! (${res.data.deletedCount} old file(s) deleted)`
            ...
            <CardHeader className="cursor-pointer hover:bg-gray-50">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {testData.listening.parts.reduce(
                      (total, part) =>
                        ...
                        <CardHeader className="cursor-pointer hover:bg-gray-50">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-lg flex items-center gap-2">
                              ...
                              <Badge variant="outline">
                                {getQuestionCount(part.questionBlocks)} / 10 questions
                              </Badge>
                            </CardTitle>
                            <div className="flex items-center gap-2">
                              <Dialog
                        ...
                        <CardHeader className="cursor-pointer hover:bg-gray-50">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-lg flex items-center gap-2">
                              ...
                              <Badge variant="outline">
                                {getQuestionCount(passage.questionBlocks)} questions
                              </Badge>
                            </CardTitle>
                            <div className="flex items-center gap-2">
                              <Dialog
            ...
            <CardHeader className="cursor-pointer hover:bg-gray-50">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {correctAnswers.correct_answers.reduce(
                      (total, section) => total + section.answers.length,
                      0
                    )}{" "}
                    answers
