...
export default function AdminUsersPage() {
  ...
  const removeTestFromUser = useCallback(
    async (userId: string, testId: string) => {
      ...
      try {
        ...
        if (res.data.success) {
          notify.success("Test assignment removed");
          // Use requestAnimationFrame to avoid DOM conflicts
          await new Promise((resolve) => requestAnimationFrame(resolve));
          await refresh();
        } else {
          notify.error("Failed to remove test assignment");
        }
      } catch (err: any) {
  ...
  const handleSaveUser = async (
    ...
    try {
      ...
      if (res.data.success) {
        ...
        notify.error(res.data.error || "Failed to update user");
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        notify.error(res.data.error || res.data.msg || "Failed to delete user");
      }
    } catch (err: any) {
  ...
  const handleBulkDelete = () => {
    if (selectedUserIds.size === 0) {
      notify.error("Please select users to delete");
      return;
    }
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        notify.success(`${selectedUserIds.size} users deleted successfully`);
        setSelectedUserIds(new Set());
        await refresh();
      } else {
        notify.error(res.data.error || "Failed to delete users");
      }
    } catch (err: any) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Defaults to 1234asdf"
                disabled={isCreating}
              />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Users List
            {hasSearchResults && (
              <StatusBadge status="info" label={`${localUsers.length} search results`} />
            )}
            {sortConfig && (
              <StatusBadge status="neutral" label={`Sorted by ${sortConfig.label}`} />
            )}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          key={user.id}
                          value={user.active_test_id || "none"}
                          onValueChange={(value) => {
                        ...
                        >
                          <SelectTrigger
                            className="w-48"
                            aria-busy={loadingUserId === user.id}
                          >
