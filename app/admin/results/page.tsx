...
export default function AdminResultsPage() {
	...
	const searchResults = useCallback(async (query: string, offset: number, limit: number) => {
	...
	const confirmDeleteResult = async () => {
		...
		try {
			...
			if (res.data.success) {
				...
				toast.error(res.data.error || 'Failed to delete result')
			}
		} catch (err: any) {
	...
	const confirmBulkDelete = async () => {
		...
		try {
			...
			if (res.data.success) {
				toast.success(`${selectedResultIds.size} results deleted successfully`)
				setSelectedResultIds(new Set())
				await refresh()
			} else {
				toast.error(res.data.error || 'Failed to delete results')
			}
		} catch (err: any) {
	...
	const gradedResults = allResultsForStats.filter(isResultCompletelyGraded)
	const emailsSent = allResultsForStats.filter(result => result.email_sent)
	const isAllSelected =
					...
					<CardContent className='p-4'>
						<div className='flex items-center space-x-2'>
							...
							<div>
								...
								<p className='text-2xl font-bold'>{selectedResultIds.size}</p>
							</div>
						</div>
					</CardContent>
				</Card>
