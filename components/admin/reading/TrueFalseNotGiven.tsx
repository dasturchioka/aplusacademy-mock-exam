...
export default function TrueFalseNotGivenAdmin({
	question,
	onChange,
}: {
	question: TFNGBlock
	onChange: (updatedBlock: TFNGBlock) => void
}) {
	const [block, setBlock] = useState<TFNGBlock>(question)
	...
	useEffect(() => {
		...
		if (needsRenumbering) {
			...
			console.log('🔢 Auto-renumbering TrueFalseNotGiven questions:', {
	...
	useEffect(() => {
		const timeout = setTimeout(() => {
			console.log('✅ TrueFalseNotGiven Component Updated:', {
				type: block.type,
	...
	const addQuestion = () => {
		...
		const newQuestion: TFNGQuestion = {
	...
	const updateQuestionField = (qIdx: number, field: keyof TFNGQuestion, value: string) => {
		const updated = [...block.questions]
		updated[qIdx][field] = value as any
		setBlock({ ...block, questions: updated })
	}

	const updateCorrectAnswer = (qIdx: number, answer: 'TRUE' | 'FALSE' | 'NOT GIVEN') => {
		const updated = [...block.questions]
