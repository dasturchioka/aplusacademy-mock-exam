				...
				{!isMatchingParagraphs ? (
					<>
						...
						<div className='flex flex-col items-start border w-auto'>
								{data.optionsHeadline ? <StableHtml as='p' html={processTextWithBoldAndCaps(data.optionsHeadline)} /> : null}
								{data.options.map(option => (
									<div key={option.variant || option.label} className='p-3'>
										<div className='p-0 flex items-center gap-2'>
											...
											<StableHtml as='div' className='text-sm text-muted-foreground' html={processTextWithBoldAndCaps(option.text)} />
										</div>
									</div>
								))}
						</div>
						...
						<div className='flex flex-col items-start border w-auto text-sm'>
								{data.pairs.map(pair => (
									<div key={pair.number} className='px-3 py-2 leading-snug'>
										...
										<StableHtml as='span' html={processTextWithBoldAndCaps(pair.item)} />
									</div>
								))}
						</div>
					</>
						...
						{data.pairs.map(pair => (
							<tr id={`qn-${pair.number}`} key={pair.number}>
								...
								>
										{pair.number}.
										{!isMatchingParagraphs && (
											<span> <StableHtml as='span' html={processTextWithBoldAndCaps(pair.item)} /></span>
										)}
								</td>
								{data.options.map(option => {
