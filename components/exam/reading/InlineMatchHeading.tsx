...
export default function InlineMatchHeading({
	...
	if (!question || !question.options || !Array.isArray(question.options)) {
		...
		return (
			<div className='space-y-6'>
				<AppAlert tone='error' title='Question unavailable'>
					Match heading question data is not available.
				</AppAlert>
			</div>
		)
	}
	...
	const DraggableOption = ({
		...
		>
			...
			{from === 'pool' ? <StableHtml as='span' html={processTextWithBoldAndCaps(` - ${option.text}`)} /> : ''}
		</div>
	)
}
...
export function ParagraphDropZone({
	...
	const DraggableOption = ({ option }: { option: Option }) => {
		...
		>
			<strong><StableHtml as='span' html={processTextWithBoldAndCaps(`${option.variant}. ${option.text}`)} /></strong>
		</div>
	)
	}
