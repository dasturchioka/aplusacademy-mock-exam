			...
			parts.forEach((part, idx) => {
				rendered.push(
					<StableHtml
						key={`p-${idx}`}
						as='span'
						html={processTextWithBoldAndCaps(part)}
					/>
				)
	...
	return (
		<div className='space-y-6'>
				...
				<div className='space-y-1'>
					{question.instructions.map((line, i) => (
						<StableHtml
							...
							html={processTextWithBoldAndCaps(line)}
						/>
					))}
				</div>
			...
			{/* Image */}
				{question.image?.url && (
					<div>
						<StableHtml as='p' className='font-semibold' html={processTextWithBoldAndCaps(question.image.headline)} />
