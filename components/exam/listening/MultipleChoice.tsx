			...
			{question.questions.map((q, idx) => (
				<div key={q.questionId} className='space-y-2'>
					<p id={`qn-${q.questionNumber}`} className='font-medium'>
						{Number(question.questionStart) + idx}. <span dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(q.questionText) }} />
					</p>
						...
						{q.options.map(opt => (
							<div key={opt.variant} className='flex items-center gap-2'>
								...
								<label htmlFor={`${q.questionId}-${opt.variant}`} className='cursor-pointer'>
									<strong>{opt.variant}.</strong> <span dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(opt.text) }} />
								</label>
							</div>
						))}
					</RadioGroup>
