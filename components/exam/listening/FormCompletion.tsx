...
export default function FormCompletionExam({
  ...
  if (question.type === "static") {
    return <div className="text-base mb-3" dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(question.text) }} />;
  }
        ...
        {question.questions.map((q, idx) => {
          ...
          return (
            ...
            >
              {q.isInteractive ? (
                <>
                  <span dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(q.text.split("____")[0]) }} />
                  <Input
                    id={`qn-${number}`}
                    className="w-40"
                    defaultValue={answer}
                    ...
                    placeholder={number}
                  />
                  <span dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(q.text.split("____")[1]) }} />
                </>
              ) : (
                <span dangerouslySetInnerHTML={{ __html: processTextWithBoldAndCaps(q.text) }} />
              )}
            </div>
          );
        })}
