		...
		<Card className='space-y-4 p-4'>
			<CardContent className='space-y-2'>
				<div className='text-sm text-muted-foreground'>
					...
					<StableHtml as='div' className='text-lg font-semibold' html={processTextWithBoldAndCaps(data.questionText)} />
				...
				{data.instructions?.map((text, idx) => (
					<StableHtml
						...
						html={processTextWithBoldAndCaps(text)}
					/>
				))}
					...
					{data.choices.map((choice, idx) => (
						...
						>
								...
								<Label className='cursor-pointer'>
									<span className='font-semibold mr-1'>{choice.variant}.</span>{' '}
									<StableHtml as='span' html={processTextWithBoldAndCaps(choice.text)} />
								</Label>
						</div>
					))}
				</div>
